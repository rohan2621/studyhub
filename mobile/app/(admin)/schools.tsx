    ...
    <View style={{ flex: 1, backgroundColor: colors.background }}>
      ...
      {isLoading ? (
        ...
        <ScrollView contentContainerStyle={{ padding: 20, gap: 12 }}
          ...
          {schools.map((s: any) => (
            <View key={s.id} style={{ backgroundColor: colors.card, borderRadius: 0, padding: 18, borderWidth: 1, borderColor: colors.border }}>
        ...
        <View style={{ flex: 1, backgroundColor: "#000000cc", justifyContent: "flex-end" }}>
          <View style={{ backgroundColor: colors.surface, borderTopLeftRadius: 0, borderTopRightRadius: 0, padding: 28 }}>
            ...
            <TextInput value={city} onChangeText={setCity} placeholder="e.g. Kathmandu" placeholderTextColor={colors.muted}
              style={{ backgroundColor: colors.inputBg, borderRadius: 0, padding: 14, color: colors.text, borderWidth: 1, borderColor: colors.border, marginBottom: 24 }} />
            <TouchableOpacity onPress={() => createMutation.mutate()} disabled={!name || !city || createMutation.isPending}
              style={{ backgroundColor: colors.primary, borderRadius: 0, padding: 16, alignItems: "center", marginBottom: 12, opacity: !name || !city ? 0.5 : 1 }}>
