        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{ marginBottom: 10 }}>
          <View style={{ flexDirection: "row", gap: 8 }}>
            <TouchableOpacity onPress={() => setYear("")}
              style={{ paddingHorizontal: 14, paddingVertical: 7, borderRadius: 20, backgroundColor: !year ? colors.primary : colors.card, borderWidth: 1, borderColor: !year ? colors.primary : colors.border }}>
            ...
            {years.map((y) => (
              <TouchableOpacity key={y} onPress={() => setYear(year === y ? "" : y)}
                style={{ paddingHorizontal: 14, paddingVertical: 7, borderRadius: 20, backgroundColor: year === y ? colors.primary : colors.card, borderWidth: 1, borderColor: year === y ? colors.primary : colors.border }}>
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator={false}>
            <View style={{ flexDirection: "row", gap: 8 }}>
              <TouchableOpacity onPress={() => setSubject("")}
                style={{ paddingHorizontal: 14, paddingVertical: 7, borderRadius: 20, backgroundColor: !subject ? colors.accent : colors.card, borderWidth: 1, borderColor: !subject ? colors.accent : colors.border }}>
              ...
              {subjects.map((s) => (
                <TouchableOpacity key={s} onPress={() => setSubject(subject === s ? "" : s)}
                  style={{ paddingHorizontal: 14, paddingVertical: 7, borderRadius: 20, backgroundColor: subject === s ? colors.accent : colors.card, borderWidth: 1, borderColor: subject === s ? colors.accent : colors.border }}>
      ...
      {isLoading ? (
        ...
        <ScrollView contentContainerStyle={{ padding: 20, gap: 12 }}
          ...
          {papers.length === 0 ? (
            ...
            <View key={paper.id} style={{ backgroundColor: colors.card, borderRadius: 20, padding: 18, borderWidth: 1, borderColor: colors.border }}>
              <View style={{ flexDirection: "row", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
                ...
                <View style={{ backgroundColor: colors.success + "22", borderRadius: 10, paddingHorizontal: 10, paddingVertical: 4 }}>
